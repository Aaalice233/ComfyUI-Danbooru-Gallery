/**
 * 简化版组执行管理器 - Simplified Group Executor Manager
 * 专注核心功能，简化架构，使用localStorage进行多选项卡协调
 */

// 简化的组执行管理器 - 不使用复杂的多选项卡锁机制
...
app.registerExtension({
    ...
    async init(app) {
        console.log('[GEM] 初始化组执行管理器');
    },

    async setup(app) {
        console.log('[SimplifiedGEM] 设置事件监听器');

        // 监听执行指令
        api.addEventListener("simplified_gem_execute", async ({ detail }) => {
            console.log('[GEM] 收到执行指令:', detail);

            const nodeId = String(detail.node_id);
            const node = app.graph._nodes_by_id[nodeId];

            if (!node) {
                console.error('[GEM] 未找到节点:', nodeId);
                return;
            }

            // 防止重复执行
            if (node.properties.isExecuting) {
                console.log('[GEM] 节点正在执行中，跳过');
                return;
            }

            // 设置执行状态
            node.properties.isExecuting = true;
            console.log('[GEM] 开始执行组列表');

            try {
                // 中断当前队列
                console.log('[GEM] 中断当前队列');
                await api.interrupt();
                ...
                console.log('[GEM] 所有组执行完成');
            } catch (error) {
                console.error('[GEM] 执行出错:', error);
                console.error('[GEM] 错误堆栈:', error.stack);
            } finally {
                // 清除执行状态
                node.properties.isExecuting = false;
                console.log('[GEM] 清除执行状态');
            }
        });
    },
    ...
    async waitForQueueEmpty() {
        console.log('[GEM] 等待队列清空');
        let attempts = 0;
        const maxAttempts = 10; // 减少等待时间，从20次改为10次
        ...
        while (attempts < maxAttempts) {
            try {
                ...
                if (running === 0 && pending === 0) {
                    console.log('[GEM] 队列已清空');
                    return;
                }

                console.log(`[GEM] 队列状态: running=${running}, pending=${pending}`);
            } catch (error) {
                console.error('[GEM] 检查队列状态失败:', error);
            }

            // 增加轮询间隔，从500ms改为1000ms
            await new Promise(resolve => setTimeout(resolve, 1000));
            attempts++;
        }

        console.warn('[GEM] 队列清空超时，继续执行');
    },

    // 执行组列表
    async executeGroupList(executionList, node) {
        console.log('[GEM] 开始执行组列表，共', executionList.length, '项');
        console.log('[GEM] 执行顺序:', executionList.map(item => item.group_name).join(' → '));

        for (let i = 0; i < executionList.length; i++) {
            const item = executionList[i];
            console.log(`[GEM] [${i + 1}/${executionList.length}] 执行:`, item);

            // 处理延迟
            if (item.group_name === '__delay__') {
                if (item.delay_seconds > 0) {
                    console.log(`[GEM] 延迟 ${item.delay_seconds} 秒`);
            ...
            if (!outputNodes || outputNodes.length === 0) {
                console.error(`[GEM] 组 "${item.group_name}" 中没有找到输出节点`);
                continue;
            }
            ...
            console.log(`[GEM] 执行组 "${item.group_name}"，节点:`, sortedNodeIds);

            // 执行节点并等待完成
            await this.executeNodes(sortedNodeIds);

            // 等待当前组执行完成
            await this.waitForExecutionComplete();

            console.log(`[GEM] 组 "${item.group_name}" 执行完成`);

            // 组间延迟
            if (item.delay_seconds > 0 && i < executionList.length - 1) {
                console.log(`[GEM] 组间延迟 ${item.delay_seconds} 秒`);
                await new Promise(resolve => setTimeout(resolve, item.delay_seconds * 1000));
            }
        }
    ...
    getGroupOutputNodes(groupName) {
        if (!app.graph || !app.graph._groups) {
            console.log(`[GEM] 没有找到组或图数据，无法获取组 "${groupName}" 的输出节点`);
            return [];
        }

        const group = app.graph._groups.find(g => g.title === groupName);
        if (!group) {
            console.warn(`[GEM] 未找到组 "${groupName}"`);
            return [];
        }

        console.log(`[GEM] 正在查找组 "${groupName}" 的输出节点...`);

        // 优化：使用filter方法一次性过滤，减少循环次数
        ...
        console.log(`[GEM] 组 "${groupName}" 内共找到 ${groupNodes.length} 个节点:`);
        ...
        console.log(`[GEM] 组 "${groupName}" 内找到 ${outputNodes.length} 个输出节点:`);
    ...
    async executeNodes(nodeIds) {
        console.log('[GEM] 执行节点:', nodeIds);

        // 确保节点ID是字符串类型，并按数字顺序排序
        ...
        console.log('[GEM] 排序后的节点ID:', sortedNodeIds);

        try {
            // 使用简化版队列管理器执行节点
            await simplifiedQueueManager.queueOutputNodes(sortedNodeIds);
        } catch (error) {
            console.error('[GEM] 执行节点失败:', error);
            throw error;
        }
    ...
    async waitForExecutionComplete() {
        console.log('[GEM] 等待执行完成');
        let attempts = 0;
        const maxAttempts = 300; // 减少最大等待次数，从600改为300
        ...
        while (attempts < maxAttempts) {
            try {
                ...
                if (running === 0 && pending === 0) {
                    console.log('[GEM] 执行完成');
                    return;
                }

                // 减少状态输出频率，从每10次改为每5次
                if (attempts % 5 === 0) {
                    console.log(`[GEM] 执行中: running=${running}, pending=${pending}`);
                }
            } catch (error) {
                console.error('[GEM] 检查执行状态失败:', error);
            }
    ...
    async beforeRegisterNodeDef(nodeType, nodeData, app) {
        ...
        nodeType.prototype.onConfigure = function (info) {
            ...
            // 恢复执行状态
            this.properties.isExecuting = info.isExecuting === true;
